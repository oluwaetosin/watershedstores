...
<?php require_once 'template/header.php'; ?>
   <div id='cart'>
   <?php echo !empty($session->cart)? "<h3><a href='". htmlentities($_SERVER["PHP_SELF"])."?emptycart=true'>Empty Cart</a></h3>" : ''; 
  $tableHead   =  '<h1>Shopping Cart</h1><table width="80%" cellspacing="0" cellpadding="5" >'.
  $tableHead  .=   '<tr bgcolor="#FFCC00">';
  $tableHead  .=   '<th scope="col" >Image</th>';
  $tableHead  .=   '<th scope="col" >Product Name</th>';
  $tableHead  .=  '<th scope="col">Unit Price</th>';
  $tableHead  .=  '<th scope="col">Quantity</th>';
  $tableHead  .=  '<th scope="col">Total Cost</th>';
  $tableHead  .=  ' <th scope="col">Remove</th>';
  $tableHead  .='</tr>';
  $tableHead  .='<tr>';
  $tableEnd   = '</tr></table>';
   echo !empty($session->cart) ? $tableHead.$cart_string.$tableEnd : '<h1>Cart Empty</h1>';
   
  
?>
<?php echo '<span class="price">'.$session->total_price().'</span><a href="checkout.php" class="checkout">Check Out</a></span><a href="index.php" class="checkout">Continue Shopping</a>'; ?>
   </div>
 </div>
</section>
